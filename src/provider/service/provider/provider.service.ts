...
export class ProviderService {
  ...
  async createProvider(createProviderDto: providerDto,idUser: number) {
    createProviderDto.createdBy = idUser;
  ...
  async replaceById(id: number, updateProviderDto: providerDto,idUser: number) {
  ...
  async removeMultiple(toDelete: number[], toDisable: number[], idUser?: number) {
    ...
    if (toDisable.length != 0) {
      if (await this.providerRepository.update(toDisable, { updatedBy: idUser, updatedAt: new Date(), active: false })) {
        resultDisable = true
      } else
        resultDisable = false
      }
